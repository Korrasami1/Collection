...
uniform int mosaic_filterSize = 1;
...
vec3 toSepiaTone(vec3 rgbSample);
 ...
 /*Mosaic effect using the filterSize to determine the amount/border*/
vec3 mosaic(vec2 coord, vec2 text2d);
...
vec3 toSepiaTone(vec3 rgbSample)
{
	...
	//-----------------------------------------------------------------
	vec3 yiqTransform1 = vec3(0.299, 0.587, 0.144);
	vec3 yiqTransform0 = vec3(0.596, -0.275, -0.321);
...
vec3 mosaic(vec2 coord, vec2 text2d) {
	...
	for(float i = 0; i <= text2d.x; i += mosaic_filterSize){
		if(coord.x >= i && coord.x <= i+mosaic_filterSize ){
			for(float j = 0; j < text2d.y; j += mosaic_filterSize){
				if(coord.y >= j && coord.y <= j+mosaic_filterSize){
					vec4 result_Texture = textureRect(frameBufferTexture, vec2(i,j));
					return result_Texture.xyz;
				}	
			}
		}
	}

	return textureRect(frameBufferTexture, coord).xyz;
}
